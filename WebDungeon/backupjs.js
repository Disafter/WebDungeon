...
var model = new WebDungeonViewModel();

ko.applyBindings(model);
////above this is javascript just before knockout induction
...
var ItemsFoundArray = new Array(false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false);
var Equipment = new Array(false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false);
...
//var timerId;
//var recoverId;
//var gameData.randomStatRoll = 0;
//var uniqueItemsFound = 0;
//var gameData.disableItemPopups = false;
...
function logIn() {
    character.name = prompt("Please enter your character's name.", "Zasheir");
    if (character.name != null) {
        document.getElementById("namebox").innerHTML = character.name;
    }
...
function loadGame() { //loadGame retrieves all user-specific data from the database. it needs to be modified to retrieve their class name once the loadClasses function is completed. it also needs to collect the user's item data once loadUserItems is completed 
    ...
    .done(function (response) {
        ...
        cLevel = userResponse.Level;
        cClass = classesResponse[userResponse.ClassID].Name;
        SQLdetail = 'SELECT';
        if (cClass) {
            $("#levelbox").text = "Level " + cLevel + " " + cClass;
        }

        gold = userResponse.Gold;
        currentFloor = userResponse.Floor;
        maxMonsterHP = (3 * userResponse.Floor) + 4;
        currMonsterHP = (3 * userResponse.Floor) + 4;
        bonusStatPoints = userResponse.BonusStatPoints;

        str = userResponse.BonusStrength; //ADD GEAR AND CLASS BONUSES TO THIS
        dex = userResponse.BonusDexterity;
        wis = userResponse.BonusIntelligence;
        luck = userResponse.BonusLuck;
        elixirs = userResponse.Elixirs;

        cHP = (1.1 * userResponse.Level) + (1.1 * userResponse.BonusStrength) + 10; //ADD IT HERE TOO
        maxHP = (1.1 * userResponse.Level) + (1.1 * userResponse.BonusStrength) + 10;
        cMP = (userResponse.BonusIntelligence + 2);
        maxMP = (userResponse.BonusIntelligence + 2);
        ...
        for (var i = 0; i < userResponse.itemIDs.length; i++) {
            ...
            if (userResponse.itemIDs[i].IsEquipped === true) {
                Equipment[userResponse.ItemID] = true;
                alert("Equipped " + itemsResponse[userResponse.itemIDs[i].ItemID].Name);
                str += itemsResponse[userResponse.itemIDs[i].ItemID].Strength;
                dex += itemsResponse[userResponse.itemIDs[i].ItemID].Dexterity;
                wis += itemsResponse[userResponse.itemIDs[i].ItemID].Intelligence;
                luck += itemsResponse[userResponse.itemIDs[i].ItemID].Luck;
                game.updatePage();
            }
